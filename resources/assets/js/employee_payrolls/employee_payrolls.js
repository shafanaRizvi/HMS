            ...
            if (result.success) {
                ...
                if (result.data.status == 1) {
                    $('#employee_payroll_status').
                        append(
                            '<span class="badge bg-light-success">'+Lang.get('js.paid')+'</span>');
                } else {
                    $('#employee_payroll_status').
                        append(
                            '<span class="badge bg-light-danger">'+Lang.get('js.unpaid')+'</span>');
                }

                $('#employee_payroll_created_on').text(moment(result.data.created_at).fromNow());
                $('#employee_payroll_updated_on').text(moment(result.data.updated_on).fromNow());
                setValueOfEmptySpan();
