...
class PatientCardTable extends LivewireTableComponent
{
    ...
    {
        $query = Patient::where('patients.tenant_id', '=', getLoggedInUser()->tenant_id)->with(['patientUser.media','SmartCardTemplate'])->whereNotNull('template_id')->whereHas('patientUser')->select('patients.*');
