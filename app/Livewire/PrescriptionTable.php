...
class PrescriptionTable extends LivewireTableComponent
{
    ...
    public function builder(): Builder
    {
        ...
        if (! getLoggedinDoctor()) {
            $query = Prescription::query()->where('prescriptions.tenant_id', '=',getLoggedInUser()->tenant_id)->select('prescriptions.*')->with('patient', 'doctor');
        } else {
            $doctorId = Doctor::where('user_id', getLoggedInUserId())->first();
            $query = Prescription::query()->where('prescriptions.tenant_id', '=',getLoggedInUser()->tenant_id)->select('prescriptions.*')->with('patient', 'doctor')->where('doctor_id',
                $doctorId->id);
        }
