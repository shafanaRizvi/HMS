...
class ManualBillPaymentTable extends LivewireTableComponent
{
    ...
    {
        $query = BillTransaction::where('bill_transactions.tenant_id', '=', getLoggedInUser()->tenant_id)->whereHas('bill.patient.patientUser')->with(['bill.patient.patientUser.media'])->select('bill_transactions.*');
