...
class AppointmentController extends AppBaseController
{
     ...
     */
    public function store_old(CreateAppointmentRequest $request): JsonResponse
    {
        $input = $request->all();
        ...
        return $this->sendResponse([
            'success' => true,
            'message' => __('messages.flash.appointment_created'),
            'redirect_url' => route('appointments.index')
        ], __('messages.flash.appointment_booked'));
    ...
    public function store(CreateAppointmentRequest $request): JsonResponse
{
    ...
    try {
        $input = $request->all();
        $input['opd_date'] = $input['opd_date'] . ' ' . $input['time']; // Added space for datetime
        $input['is_completed'] = isset($input['status']) ? Appointment::STATUS_COMPLETED : Appointment::STATUS_PENDING;
        $input['payment_type'] = $input['payment_type'] ?? 4;
        ...
        $jsonFields = array_filter($input, fn($key) => strpos($key, 'field') === 0, ARRAY_FILTER_USE_KEY);
        $input['custom_field'] = !empty($jsonFields) ? $jsonFields : null;

        // Check if the email already exists before creating an appointment
        $existingUser = User::where('email', $input['email'] ?? null)->first();
        if ($existingUser) {
        ...
        // Payment handling logic (You may also want to wrap this in a transaction or handle errors)
        if (in_array($input['payment_type'], [1, 2, 3, 5])) {
            ...
            return $this->sendResponse([
                ...
                'payment_type' => $input['payment_type'],
                'amount' => $input['appointment_charge'],
            ], ucfirst($input['payment_type']) . ' session created successfully');
        } elseif (in_array($input['payment_type'], [7, 8])) {
            $payStackData = [
                'payment_type' => $input['payment_type'],
                'amount' => $input['appointment_charge'],
                'input' => $input,
        ...
        $this->sendAppointmentEmails($input, $data);
