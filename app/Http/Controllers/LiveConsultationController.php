...
class LiveConsultationController extends AppBaseController
{
    ...
    public function zoomCallback_old(Request $request)
    {
        $token = $request->get('code');
        $zoomRepo = FacadesApp::make(ZoomRepository::class);
        $res = $zoomRepo->connectWithZoom($token);
    ...
    public function zoomCallback(Request $request)
    {
        try {
            ...
            if (!$authorizationCode) {
                ...
                return redirect()->route('live.consultation.index')->withErrors('Authorization code is missing.');
            }
    
            Log::info('Authorization code received:', ['code' => $authorizationCode]);
            ...
            if ($response->failed()) {
                Log::error('Failed to retrieve access token from Zoom:', ['response' => $response->body()]);
                return redirect()->route('live.consultation.index')->withErrors('Failed to retrieve access token from Zoom.');
            }
    
            $tokenData = $response->json();
            Log::info('Zoom access token retrieved:', ['tokenData' => $tokenData]);
    
            $meetingResponse = Http::withToken($tokenData['access_token'])->post('https://api.zoom.us/v2/users/me/meetings', [
            ...
            if ($meetingResponse->failed()) {
                ...
                return redirect()->route('live.consultation.index')->withErrors('Failed to create Zoom meeting.');
            }
    
            $meetingData = $meetingResponse->json();
            Log::info('Zoom meeting created:', ['meetingData' => $meetingData]);
    
            return redirect($meetingData['join_url']);
    
        } catch (\Exception $e) {
            // Handle exceptions and log the error
            Log::error('Error connecting with Zoom:', ['error' => $e->getMessage()]);
            return redirect()->route('live.consultation.index')->withErrors('Failed to connect with Zoom.');
        }
    }
