...
class SmartPatientCardController extends AppBaseController
{
    ...
    {
        $cardTemplates = SmartPatientCard::whereTenantId(getLoggedInUser()->tenant_id)->pluck('template_name','id')->toArray();
    ...
    public function sendSuccess($message = '', $status = 200): JsonResponse
    {
        return response()->json([
            'success' => true,
            'message' => $message
        ], (int) $status); // Ensure $code is cast to integer
    }
    ...
    public function smartCardStore(Request $request)
    {
        ...
        if($input['patient'] == "all"){
            ...
            foreach($patients as $patient){
                if(empty($patient->patient_unique_id)){
                    $patient->update(['patient_unique_id' => Str::random(6)]);
                }
                $patient->update(['template_id' => $input['template_id']]);
            }
        }
        if($input['patient'] == "one"){
        ...
        return $this->sendSuccess(__('messages.lunch_break.smart_card_saved'),200);
    }
